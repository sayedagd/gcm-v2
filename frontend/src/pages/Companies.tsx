                      ...
                      render: (val, row) => (
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col min-w-0">
                            <Link to={`/c?id=${row.company_id}`} onClick={e => e.stopPropagation()} className="font-bold text-text-main text-sm truncate uppercase tracking-tight hover:text-primary hover:underline focus:outline-none">{val || '---'}</Link>
                            <span className="text-[8px] font-bold text-text-subtle uppercase tracking-widest">{row.commercial_reg}</span>
                          </div>
                        </div>
                      )
                    },
                    { key: 'commercial_reg', label: isAr ? 'السجل' : 'CR NO', render: (v) => <span className="font-mono text-xs font-bold text-text-subtle">{v}</span> },
                    { key: 'vat_no', label: isAr ? 'الضريبة' : 'VAT', render: (v) => <span className="font-mono text-xs font-bold text-success">{v}</span> },
                    { key: 'projects_count', label: isAr ? 'المواقع' : 'SITES', render: (_, row) => <span className="px-3 py-1 bg-primary/10 text-primary rounded-lg font-bold text-xs">{projects.filter(p => p.company_id === row.company_id).length}</span> },
                    { key: 'contact', label: isAr ? 'المسؤول' : 'CONTACT', render: (_, row) => <div className="space-y-0.5"><p className="font-bold text-text-main text-xs">{row.contact_name}</p><p className="text-[10px] text-text-subtle font-bold">{row.contact_phone}</p></div> },
                    {
                      key: 'actions',
                      label: '',
                      render: (_, row) => (

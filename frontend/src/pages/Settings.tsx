              ...
              <Card className="p-8 lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    ...
                    <Input value={saasConfig.aiAssistant?.nameAr || 'شادي'} onChange={val => updateSaaS({ aiAssistant: { ...saasConfig.aiAssistant, nameAr: val } })} dir="rtl" />
                  </div>
                  <div className="space-y-4">
                    <label className="text-[10px] font-black uppercase text-text-subtle tracking-widest">{isAr ? 'الاسم (إنجليزي)' : 'Name (EN)'}</label>
                    <Input value={saasConfig.aiAssistant?.name || 'Shady'} onChange={val => updateSaaS({ aiAssistant: { ...saasConfig.aiAssistant, name: val } })} dir="ltr" />
                  </div>
                </div>
              </Card>
            </div>

...
const Projects: React.FC = () => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter(p =>
      ...
      (companyMap[p.company_id]?.company_name || '').toLowerCase().includes(q)
    );
  ...
  const budgetProgressMap = useMemo(() => {
    ...
    projects.forEach(p => {
      const committedCost = projectServices
        .filter(ps => ps.project_id === p.project_id)
        .reduce((sum, ps) => {
          const totalCost = Number(ps.total_cost);
          if (Number.isFinite(totalCost) && totalCost > 0) return sum + totalCost;
      ...
      const baselineBudget = Number(p.budget) || committedCost || Number(p.total_quantities) || 0;
      map[p.project_id] = baselineBudget > 0 ? Math.min(100, Math.round((committedCost / baselineBudget) * 100)) : 0;
    });
    return map;
  }, [projects]);
  ...
  const quantityProgressMap = useMemo(() => {
    const map: Record<string, number> = {};
    projects.forEach(p => {
      let target = Number(p.total_quantities) || 0;
      if (target <= 0) {
        // Fallback: Sum of service quantities
      ...
      if (target <= 0) { map[p.project_id] = 0; return; }
      const projectTrips = trips.filter(t => t.project_id === p.project_id);
      const actual = projectTrips.reduce((acc, t) => acc + (Number(t.quantity) || 0), 0);
      const progress = Math.round((actual / target) * 100);
      map[p.project_id] = progress > 100 ? 100 : progress;
    });
    return map;
  }, [projects, trips, projectServices]);

  const calculateBudgetProgress = (projectId: string) => budgetProgressMap[projectId] || 0;
  const calculateQuantityProgress = (projectId: string) => quantityProgressMap[projectId] || 0;
  ...
  const projectStats = useMemo(() => {
    ...
    projects.forEach(p => {
      ...
      // Quantity Progress check
      if (quantityProgressMap[p.project_id] >= 100) {
        endedDueToQuantity++;
      }

      // Approaching limit (at service level)
      const pServices = projectServices.filter(ps => ps.project_id === p.project_id);
      let isApproaching = false;

      pServices.forEach(ps => {
        if (p.status === 'ACTIVE') totalActiveServices++;
        
        const serviceTrips = trips.filter(t => t.project_id === p.project_id && t.service_id === ps.service_id);
        const serviceActual = serviceTrips.reduce((acc, t) => acc + (Number(t.quantity) || 0), 0);
        const warningThreshold = Number(ps.warning_threshold) || 0;
        const serviceTarget = Number(ps.quantity) || 0;
  ...
  }, [projects, projectServices, trips, quantityProgressMap]);
                ...
                <Table
                  ...
                  columns={[
                      ...
                      render: (val, row) => (
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col min-w-0">
                            <Link to={`/p?id=${row.project_id}`} onClick={e => e.stopPropagation()} className="font-bold text-text-main text-sm truncate uppercase tracking-tight hover:text-primary hover:underline focus:outline-none">{val}</Link>
                    ...
                    },
                    {
                      ...
                      render: (_, row) => <span className="font-bold text-xs text-text-subtle uppercase tracking-tight">{companyMap[row.company_id]?.company_name}</span>
                    },
                    {
                      key: 'status',

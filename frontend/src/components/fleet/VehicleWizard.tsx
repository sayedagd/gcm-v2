...
const VehicleWizard: React.FC<VehicleWizardProps> = ({
    ...
    const updateDocument = (idx: number, key: keyof VehicleDocument, value: any) => {
        ...
        if (key === 'expiry_date') {
            ...
            if (value === '' || diffDays < 0) {
                newDocs[idx].status = DocumentStatus.EXPIRED;
            } else if (diffDays <= 30) {
                newDocs[idx].status = DocumentStatus.NEAR_EXPIRY;
            } else {
                newDocs[idx].status = DocumentStatus.ACTIVE;
            }
        }
                            ...
                            {localVehicle?.photo_front ? (
                                <>
                                    ...
                                    <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 flex items-center justify-center gap-2 transition-all">
                                        ...
                                        <button onClick={() => setLocalVehicle(prev => ({ ...prev, photo_front: undefined }))} className="p-2 bg-rose-500/80 hover:bg-rose-600 rounded-lg text-white"><X size={18} /></button>
                                    </div>
                                </>
                            ) : (
                            ...
                            {localVehicle?.photo_back ? (
                                <>
                                    ...
                                    <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 flex items-center justify-center gap-2 transition-all">
                                        ...
                                        <button onClick={() => setLocalVehicle(prev => ({ ...prev, photo_back: undefined }))} className="p-2 bg-rose-500/80 hover:bg-rose-600 rounded-lg text-white"><X size={18} /></button>
                                    </div>
                                </>
                            ) : (

...
const translateNotif = (n: AppNotification, isAr: boolean) => {
    ...
    if (title.includes(':')) {
        ...
        const transAction = dictionary[action] ? dictionary[action][isAr ? 'ar' : 'en'] : action;
        title = isAr ? `${transAction}: ${name}` : `${transAction}: ${name}`;
    } else if (dictionary[title]) {
        title = dictionary[title][isAr ? 'ar' : 'en'];
    }

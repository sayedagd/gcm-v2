...
export const getMenuGroups = (t: any, isAr: boolean, role: Role) => {
    ...
    return [
    {
        ...
        items: [
            { name: t.dashboard, href: '/db', icon: LayoutDashboard, roles: [Role.ADMIN, Role.ACCOUNTANT, Role.DATA_ENTRY, Role.DRIVER] },
            { name: t.companies, href: '/c', icon: Building2, roles: [Role.ADMIN] },
            { name: t.projects, href: '/p', icon: Briefcase, roles: [Role.ADMIN] },
            { name: t.trips, href: '/t', icon: ClipboardList, roles: [Role.ADMIN, Role.DATA_ENTRY, Role.LOGISTICS, Role.REPORTS_MANAGER, Role.DRIVER] },
            { name: isAr ? 'لوحة التقارير' : 'Reports Hub', href: '/rd', icon: BarChart3, roles: [Role.ADMIN, Role.REPORTS_MANAGER] }
        ]
    },
    {
        title: t.logistics,
        ...
        items: [
            { name: t.fleet, href: '/f', icon: Truck, roles: [Role.ADMIN, Role.DATA_ENTRY, Role.LOGISTICS] },
            { name: t.inventory, href: '/iv', icon: Package, roles: [Role.ADMIN, Role.DATA_ENTRY, Role.LOGISTICS] },
            { name: t.drivers, href: '/dr', icon: HardHat, roles: [Role.ADMIN, Role.DATA_ENTRY, Role.LOGISTICS] },
            { name: isAr ? 'قائمة الطلبات والتتبع' : 'Trip Queue & Tracking', href: '/logistics/queue', icon: MapPin, roles: [Role.ADMIN, Role.LOGISTICS, Role.REPORTS_MANAGER] },
            { name: t.suppliers, href: '/sup', icon: Building2, roles: [Role.ADMIN, Role.LOGISTICS] },
            { name: t.facilities, href: '/fac', icon: Building2, roles: [Role.ADMIN, Role.REPORTS_MANAGER] }
        ]
    },
    {
        title: t.admin,
        ...
        items: [
            { name: t.accounting, href: '/acc', icon: BarChart3, roles: [Role.ADMIN, Role.ACCOUNTANT] },
            { name: t.users, href: '/u', icon: Users, roles: [Role.ADMIN] },
            { name: t.services, href: '/s', icon: Box, roles: [Role.ADMIN, Role.REPORTS_MANAGER] },
            { name: t.logs, href: '/l', icon: ScrollText, roles: [Role.ADMIN, Role.DATA_ENTRY, Role.LOGISTICS, Role.REPORTS_MANAGER] },
            { name: t.settings, href: '/st', icon: Box, roles: [Role.ADMIN] },
            { name: t.monitor, href: '/sys', icon: Activity, roles: [Role.ADMIN] },
            { name: t.landing, href: '/le', icon: LayoutIcon, roles: [Role.ADMIN] },
            { name: isAr ? 'إدارة المتجر' : 'Store Management', href: '/store-admin', icon: Package, roles: [Role.ADMIN] },

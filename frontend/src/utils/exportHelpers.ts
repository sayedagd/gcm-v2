...
export const generateBulkPdf = async (
    ...
    const mergedTemplateConfig: TemplateConfig = {
        ...templateConfig,
        global: { ...templateConfig?.global, accentColor: options.accentColor || templateConfig?.global?.accentColor }
    };
...
export const copyAIPrompt = async (trips: Trip[], isAr: boolean) => {
    ...
    const startDate = trips.length > 0 ? trips[0].date : 'N/A';
    const endDate = trips.length > 0 ? trips[trips.length - 1].date : 'N/A';

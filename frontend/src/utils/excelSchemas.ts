...
export interface ExcelSchema<T = any> {
   ...
   */
  buildEntity: (mapped: Partial<T>, row: Record<string, any>) => T;
  /**
...
export const vehicleSchema: ExcelSchema<Vehicle> = {
  ...
  buildEntity: (mapped, row) => {
    ...
    return {
      ...
      permit_zones:   (mapped.permit_zones as string) || undefined,
...
export const driverSchema: ExcelSchema<Driver> = {
  ...
  buildEntity: (mapped) => ({
    ...
    role_title:          (mapped.role_title         as string) || undefined,
    ...
    license_expiry:      (mapped.license_expiry     as string) || undefined,
    iqama_no:            (mapped.iqama_no           as string) || undefined,
    iqama_expiry:        (mapped.iqama_expiry       as string) || undefined,
    operating_card_no:   (mapped.operating_card_no  as string) || undefined,
    operating_card_expiry:(mapped.operating_card_expiry as string) || undefined,
    insurance_no:        (mapped.insurance_no       as string) || undefined,
    insurance_expiry:    (mapped.insurance_expiry   as string) || undefined,
    permit_count:        Number(mapped.permit_count) || 0,
    permit_zones:        (mapped.permit_zones       as string) || undefined,

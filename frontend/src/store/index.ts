...
const getApiBaseUrl = () => {
  const runtimeUrl = (window as any).GCM_CONFIG?.API_BASE_URL;
...
const ENTITY_ROUTE_MAP: Record<string, string> = {
  ...
  [EntityType.SERVICE]: '/s',
  [EntityType.VEHICLE]: '/f',
  [EntityType.DRIVER]: '/dr',
  [EntityType.SUPPLIER]: '/sup',
  [EntityType.FACILITY]: '/fac',
  [EntityType.CONTAINER]: '/iv',
  [EntityType.TANK]: '/iv',
  [EntityType.SCALE]: '/iv',
  [EntityType.SIZE]: '/iv',
  [EntityType.USER]: '/u',
  [EntityType.LANDING]: '/le',
};
...
export const useGCMStore = create<GCMStore>((set, get) => ({
  darkMode: localStorage.getItem('gcm_dark_mode') === 'true',
  booting: true,
  ...
  loadAllData: async () => {
    const { api } = get();
    const safeFetch = async (fetcher: () => Promise<any>, setter: (d: any) => void, fallback: any = []) => {
      try {
    ...
    try {
      await Promise.allSettled([
        safeFetch(api.getCompanies, (d) => set({ companies: d })),
        safeFetch(api.getProjects, (d) => set({
        ...
        safeFetch(api.getTrips, (d) => set({ allTrips: (d || []).map((t: any) => ({ ...t, date: t.date?.split('T')[0] || t.date || '' })) })),
        safeFetch(api.getUsers, (d) => set({ users: d })),
        safeFetch(api.getServices, (d) => set({ services: d })),
        safeFetch(api.getVehicles, (d) => set({
        ...
        safeFetch(api.getDrivers, (d) => set({
        ...
        safeFetch(api.getContainers, (d) => set({ containers: d })),
        safeFetch(api.getTanks, (d) => set({ tanks: d })),
        safeFetch(api.getScales, (d) => set({ scales: d })),
        safeFetch(api.getInventorySizes, (d) => set({ inventorySizes: d })),
        safeFetch(api.getLogs, (d) => set({ logs: d })),
        safeFetch(api.getNotifications, (d) => set({ notifications: (d || []).map((n: any) => ({ ...n, isRead: n.read })) })),
        safeFetch(api.getPermissionRequests, (d) => set({ permissionRequests: (d || []).map((r: any) => ({ ...r, fromLocation: r.from_location })) })),
        safeFetch(api.getContactSubmissions, (d) => set({ contactSubmissions: d })),
        safeFetch(api.getProjectServices, (d) => set({ projectServices: d })),
        safeFetch(api.getSuppliers, (d) => set({ suppliers: d })),
        safeFetch(api.getEquipments, (d) => set({ environmentalEquipments: d })),
        safeFetch(api.getEquipmentInquiries, (d) => set({ equipmentInquiries: d })),
        safeFetch(api.getFacilities, (d) => set({
        ...
        safeFetch(api.getAssetRequests, (d) => set({ assetRequests: d })),
        safeFetch(api.getAssetServiceLinks, (d) => set({ assetServiceLinks: d }))
      ]);
    } catch (e) {
  ...
  addPermissionRequest: async (data) => {
    const { api } = get();
    try {
      if ((api as any).addPermissionRequest) {
        await (api as any).addPermissionRequest(data);
      } else {
        // Fallback or optimistic update if API doesn't support it yet
        const newReq = {
          id: `REQ-${Date.now()}`,
          ...data,
        ...
        set(state => ({ permissionRequests: [newReq, ...state.permissionRequests] }));
      }
    } catch (err) {
  ...
  updateRequestStatus: async (id, status) => {
    const { api } = get();
    try {
      if ((api as any).updatePermissionRequestStatus) {
        await (api as any).updatePermissionRequestStatus(id, status);
      }
      set(state => ({
  ...
  deletePermissionRequest: async (id) => {
    const { api } = get();
    try {
      if ((api as any).deletePermissionRequest) {
        await (api as any).deletePermissionRequest(id);
      }
      set(state => ({
  ...
  processAssetRequest: async (id, status, notes) => {
    ...
    try {
      ...
      const updatedReq = { ...target, status, notes: notes || target.notes };

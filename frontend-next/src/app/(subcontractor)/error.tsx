...
export default function SubcontractorError({
  ...
  useEffect(() => {
    console.error("[SubcontractorRouteError]", error);
  }, [error]);

  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="w-full max-w-xl rounded-3xl border border-border bg-surface p-8 text-center space-y-4">
        <h2 className="text-2xl font-black text-text-main">Subcontractor page failed to load</h2>
        <p className="text-sm text-text-subtle">
          An unexpected issue occurred while rendering this subcontractor workspace page.
        </p>
        <div className="flex items-center justify-center gap-3 pt-2">
          ...
          <button
            onClick={() => router.push("/subcontractor/dashboard")}

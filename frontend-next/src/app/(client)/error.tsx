...
export default function ClientError({
  ...
  useEffect(() => {
    console.error("[ClientRouteError]", error);
  }, [error]);

  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="w-full max-w-xl rounded-3xl border border-border bg-surface p-8 text-center space-y-4">
        <h2 className="text-2xl font-black text-text-main">Client page failed to load</h2>
        <p className="text-sm text-text-subtle">
          An unexpected issue occurred while rendering this client workspace page.
        </p>

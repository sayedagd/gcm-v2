...
type ServiceTreeNode = Service & { children: ServiceTreeNode[] };
...
const Services: React.FC = () => {
  ...
  const serviceTree = useMemo(() => {
    const buildTree = (parentId: string | null = null): ServiceTreeNode[] => {
      return services
        .filter(s => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await upsertService({
        ...
        ...(currentService.major_category ? { major_category: currentService.major_category } : {}),
        ...(currentService.parent_id ? { parent_id: currentService.parent_id } : {}),

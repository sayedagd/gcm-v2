...
import {
  ...
  Zap, ShieldCheck, LayoutGrid, List as ListIcon, DownloadCloud, Building2, ChevronLeft, ChevronRight, Eye, Edit2, Trash2, AlertCircle
} from 'lucide-react';
import {
...
const Companies: React.FC = () => {
  ...
  }, [filteredCompanies, currentPage, indexOfFirstItem, indexOfLastItem]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setFormError(error?.messageAr || error?.messageEn || error?.message || (isAr ? 'خطأ أثناء الحفظ' : 'Error saving company'));
    } finally {
                      ...
                      render: (val: any, row: any) => (
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-surface-subtle border border-border flex items-center justify-center overflow-hidden shrink-0">
                            {row.logo_url ? <img src={row.logo_url} onError={handleImageError} className="w-full h-full object-contain" /> : <span className="text-sm font-bold text-primary">{(val || 'C').charAt(0)}</span>}
                          </div>
                          <div className="flex flex-col min-w-0">
                            <Link href={`/c?id=${row.company_id}`} onClick={e => e.stopPropagation()} className="font-bold text-text-main text-sm truncate uppercase tracking-tight hover:text-primary hover:underline focus:outline-none">{val || '---'}</Link>
                            <span className="text-[8px] font-bold text-text-subtle uppercase tracking-widest">{row.commercial_reg}</span>
                          </div>
                        </div>
                      )
                    },
                    { key: 'commercial_reg', label: isAr ? 'السجل' : 'CR NO', render: (v: any) => <span className="font-mono text-xs font-bold text-text-subtle">{v}</span> },
                    { key: 'vat_no', label: isAr ? 'الضريبة' : 'VAT', render: (v: any) => <span className="font-mono text-xs font-bold text-success">{v}</span> },
                    { key: 'projects_count', label: isAr ? 'المواقع' : 'SITES', render: (_: any, row: any) => <span className="px-3 py-1 bg-primary/10 text-primary rounded-lg font-bold text-xs">{projects.filter(p => p.company_id === row.company_id).length}</span> },
                    { key: 'contact', label: isAr ? 'المسؤول' : 'CONTACT', render: (_: any, row: any) => <div className="space-y-0.5"><p className="font-bold text-text-main text-xs">{row.contact_name}</p><p className="text-[10px] text-text-subtle font-bold">{row.contact_phone}</p></div> },
                    {
                      key: 'actions',
                      label: '',

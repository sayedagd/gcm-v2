...
const TripDetailsModal = dynamic(() => import('@/components/trips/TripDetailsModal'));
const TripWizard = dynamic(() => import('@/components/trips/TripWizard'));
const SignatureApproveModal = dynamic(() =>
   import('@/components/ui/SignatureApproveModal').then((mod) => mod.SignatureApproveModal),
);

const STATUS_TABS = ['ALL', 'REQUESTED', 'IN_PROGRESS', 'PENDING_DOCS', 'PENDING_REVIEW', 'COMPLETED'] as const;
const DATE_FILTERS = ['ALL', 'TODAY', 'YESTERDAY', 'TWO_DAYS_AGO', 'LAST_WEEK', 'CUSTOM'] as const;
...
const Trips: React.FC = () => {
   const {
      trips, services, upsertTrip,
      deleteTrip, saasConfig, currentUser, exportEnabled, addNotification, booting
   } = useStore();

   const { fetchRates: fetchSupplierRates } = useSupplierRates();
   ...
   const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!file) return;
      try {
         const { added } = await importFromSchema(file, tripSchema, trips, upsertTrip);
                              ...
                              if (filter === 'CUSTOM') {
                                 ...
                                 input.onchange = (event: Event) => {
                                    const target = event.target as HTMLInputElement | null;
                                     if (target?.value) {
                                        handleCustomDateChange(target.value);
                                     }
                                 };
                                 if (input.showPicker) {
                                    input.showPicker();
               ...
               <div className="flex flex-wrap items-center gap-2 shrink-0 xl:ml-auto w-full xl:w-auto px-1 pb-1 xl:p-0">
                  <div className="bg-surface px-4 py-1.5 rounded-2xl border border-border shadow-sm flex items-center gap-3 transition-all hover:border-primary shrink-0">
                     ...
                     <div className="flex flex-col text-right">
                        ...
                        <select aria-label={translations.materialFilter} className="bg-transparent border-none outline-none font-bold text-xs min-w-[140px] p-0 cursor-pointer text-text-main" value={selectedServiceId} onChange={e => handleServiceFilterChange(e.target.value)}>
               ...
               columns={[
                  {
                     ...
                     render: (_, trip: Trip) => (
                        <div className="flex flex-col gap-1">
                           <Link href={`/t?id=${trip.trip_id}`} onClick={e => e.stopPropagation()} className="flex items-center gap-2 hover:opacity-80 transition-opacity focus:outline-none w-fit">
                  ...
                  },
                  {
                     ...
                     render: (val: unknown) => {
                        const projectId = typeof val === 'string' ? val : '';
                        const project = projectMap[projectId];
                        const companyId = project?.company_id;
                        const company = companyId ? companyMap[companyId] : undefined;
                        return (
                           <div className="space-y-1">
                              <Link href={`/projects?id=${project?.project_id}`} className="font-bold text-text-main flex items-center gap-2 hover:text-primary transition-colors hover:underline">
                                 <Briefcase size={14} className="text-primary" />
                                 {project?.project_name || projectId || '---'}
                              </Link>
                              <Link href={`/companies?id=${company?.company_id}`} className="text-[10px] font-bold text-text-subtle tracking-widest uppercase hover:text-primary transition-colors block">
                  ...
                  },
                  {
                     ...
                     render: (val: unknown) => {
                        const serviceId = typeof val === 'string' ? val : '';
                        const srv = serviceMap[serviceId];
                        return (
                           <div className="flex items-center gap-2">
                              ...
                              <span className="font-bold text-[11px] text-text-main">{srv?.service_name || serviceId || '---'}</span>
                           </div>
                        );
                     }
                  },
                        ...
                        <div className="space-y-2">
                           <div className="flex items-center gap-2">
                              ...
                              <div className="flex flex-col">
                                 ...
                                 <Link href={`/drivers?id=${trip.driver_id}`} className="font-bold text-text-main text-[11px] hover:text-primary hover:underline transition-colors">
                           ...
                           <div className="flex items-center gap-2">
                              ...
                              <div className="flex flex-col">
                                 ...
                                 <Link href={`/fleet?id=${trip.vehicle_id}`} className="font-bold text-text-main text-[11px] hover:text-primary hover:underline transition-colors">
                  ...
                  },
                  {
                     ...
                     render: (val: unknown, trip: Trip) => {
                        const quantity = typeof val === 'number' || typeof val === 'string' ? val : 0;
                        return (
                           <div className="flex flex-col items-center">
                              <span className="text-2xl font-bold text-text-main tracking-tight">{formatNumber(quantity)}</span>

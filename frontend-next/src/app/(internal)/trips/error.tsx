...
export default function TripsError({
  ...
  useEffect(() => {
    console.error("[TripsFeatureError]", error);
  }, [error]);

  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="w-full max-w-xl rounded-3xl border border-border bg-surface p-8 text-center space-y-4">
        <h2 className="text-2xl font-black text-text-main">Trips page failed to load</h2>
        <p className="text-sm text-text-subtle">
          We could not load trip operations right now. You can retry or return safely.
        </p>

...
const UserManagement: React.FC = () => {
  ...
  const handleApproveFinal = async () => {
    ...
    try {
      const nameFromEmail = approvalTarget.email.split('@')[0] || approvalTarget.email;
      ...
      await upsertUser({
        ...
        role: approveRole,
        ...((approveRole === Role.COMPANY_USER || approveRole === Role.PROJECT_USER || approveRole === Role.CLIENT) && approveCompany ? { company_id: approveCompany } : {}),
        ...(approveRole === Role.PROJECT_USER && approveProject ? { project_id: approveProject } : {}),
        avatar: '/logo.png'
      });
                ...
                columns={[
                  {
                    key: 'email',
                    label: t.table.email,
                    render: (val: any, req: PermissionRequest) => (
                  ...
                  },
                  {
                    key: 'id',
                    label: t.table.date,
                    render: (_: any, req: PermissionRequest) => (
                      <div className="flex items-center gap-2 text-text-subtle font-bold">

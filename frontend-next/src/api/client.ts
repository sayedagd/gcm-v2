...
export const createApiClient = (baseUrl: string = '') => {
    ...
    return {
        ...
        deletePermissionRequest: (id: string) => request(`/api/permission-requests/${id}`, { method: 'DELETE' }),
        ...
        getBackupStatus: () => request('/api/system/backup/status'),
        restoreBackup: (file: File) => {
            const formData = new FormData();
            formData.append('backup_file', file);
            
            const url = buildApiUrl(baseUrl || '', '/api/system/backup/restore');
            const isAuth = localStorage.getItem('gcm_auth_session') === 'true';
            const token = isAuth ? localStorage.getItem('gcm_jwt_token') || '' : '';

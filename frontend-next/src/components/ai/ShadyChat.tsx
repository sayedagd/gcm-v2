...
const ShadyChat: React.FC = () => {
    ...
    useEffect(() => {
        if (isOpen && messages.length === 0) {
            ...
            const actions: { label: string; value: string }[] = [
                ...
                { label: isAr ? '🚚 إدارة الأسطول' : '🚚 Fleet Management', value: '/fleet' },
                { label: isAr ? '📦 المخزون' : '📦 Inventory', value: '/inventory' },
        ...
        try {
            ...
            addAssistantMessage(
                    ...
                    : `✅ **Facility added successfully!** 🎉\n\n Name: **${facility.name}**\n ID: \`${facId}\``,
                {
                    type: 'quick_actions',
                    actions: [
                        { label: isAr ? '🏗️ عرض المرافق' : '🏗️ View Facilities', value: '/facilities' },
    ...
    const handleAction = (value: string) => {
        ...
        if (value === 'سجل مرفق جديد' || value === 'New Facility' || value === '/facilities') {
            startFacilityFlow();
            return;
        }

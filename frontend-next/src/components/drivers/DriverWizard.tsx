...
const DriverWizard: React.FC<DriverWizardProps> = ({
    ...
    const updatePermit = (idx: number, field: keyof PermitEntry, val: string) => {
        setPermits(prev => {
            const next = [...prev];
            const current = next[idx] || { no: '', zone: '' };
    ...
    const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = () => {
            ...
            if (uploadType === 'permit' && activePermitIdx !== null) {
                setPermits(prev => {
                    const next = [...prev];
                    const current = next[activePermitIdx] || { no: '', zone: '' };
                    next[activePermitIdx] = { ...current, fileName: file.name, fileData: base64 };
                    return next;
                });
            ...
            <div className="p-6 bg-surface border border-border space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Select
                        ...
                        onChange={val => update('vehicle_id', val || undefined)}
